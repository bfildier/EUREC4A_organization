...
class RadiativeScaling():
    ...
    def kappa(self,nu,band='rot'):
        ...
        kappa_rot = k_rot*np.exp(-(nu/1e2-nu_rot)/l_rot) # m2/kg, or mm-1
        ...
        kappa_vr = k_vr*np.exp(-(nu_vr-nu/1e2)/l_vr) # m2/kg, or mm-1
    ...
    def nu(self,kappa,band='rot'):
        ...
        if band == 'rot':
            ...
            return 1e2*(nu_rot+l_rot*np.log(k_rot/kappa)) # m-1
        ...
        # vr band
            ...
            return 1e2*(nu_vr-l_vr*np.log(k_vr/kappa)) # m-1
    ...
    def computeSpectralIntegral(self,band='rot'):
        ...
        if band == 'rot':
            self.nu_array = np.linspace(20000,80000) # m-1
        elif band == 'vr':
